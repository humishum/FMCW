...
def Fourier(x,fs,NFFT):     # Approximate Fourier transform on the interval (-fs/2,fs/2)
    ...
    f=(-.5+k/float(NFFT))*fs
    n=np.arange(N)
    return f, T/N*np.exp(1j*np.pi*f*T)*fft(x*(-1.)**n,NFFT)
...
# plt.title('Cubic phase error (maximum frequency error of %.1f kHz)' % (df_max/1.e3))
# plt.title('Quartic phase error (maximum frequency error of %.1f kHz)' % (df_max/1.e3))

plt.xlim(np.array([fb-10.e4,fb+10.e4])/1.e6)
plt.ylim([-100,0])
